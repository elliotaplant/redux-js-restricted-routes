...
const mapStateToAuthProps = ({auth: {
    isAuthed
  }}) => ({
  restricted: !isAuthed
})
const ConnectedResource = connect(mapStateToAuthProps)(Resource);
...
function Topic({match}) {
  ...
  return (<div>
    ...
    <ul>
      {
        topic.resources.map((sub) => (<li key={sub.id}>
    ...
    <Route path={`${match.path}/:subId`} component={ConnectedResource}/>
  </div>)
}
